...
def custom_ODI_bowl(bowl, name, oname):
    # Load the dataset
    df_bowl = pd.read_csv('bowling_test.csv')
    ...
    if len(filtered_df_bowl) < num_players:
        ...
        joblib.dump(model_bowl, 'custom_bowling_testmodel.joblib')

        # Make predictions using the loaded model
        loaded_model_bowl = joblib.load('custom_bowling_testmodel.joblib')
